...
public class CheckingReport extends HttpServlet {
	...
	private Map<String, Comparable> mapParameters(HttpServletRequest request) {
	    ...
	    parameters.put("P_PLATE",StringUtil.Unicode2ASCII(request.getParameter("license")));
//	    parameters.put("SMONTH",request.getParameter("coachworkno"));
//	    parameters.put("EMONTH",request.getParameter("motorno"));
	   return parameters;
	...
	private void generatePDFOutput(HttpServletRequest request, HttpServletResponse response ) throws JRException, NamingException, SQLException, IOException, ServletException {
		...
		System.out.println("LICENSE : "+params.get("P_PLATE"));
		System.out.println();
		if (params == null) {
	           request.setAttribute("url", "pages/service/CheckingRP.jsp");
			...
			in = request.getSession().getServletContext().getResourceAsStream(this.pathFile+"CheckingRP.jasper");
	    	...
	    	String fileName = "Checking" + df.format(calendar.getTime()) + ".pdf";
