...
switch(languageCode){
	case "th":	/* English */
		...
		var monthArrayShort = ['ม.ค.','ก.พ.','มี.ค.','เม.ษ.','พ.ค.','มิ.ย.','ก.ค.','ส.ค.','ก.ย.','ต.ค.','พ.ย.','ธ.ค.'];
		var dayArray = ['จ.','อ.','พ.','พฤ.','ศ.','ส.','อา.'];
		var weekString = 'สัปดาห์ที่';
...
function selectYear()
{
	...
	currentYear = (this.innerHTML.replace(/[^\d]/g,'')*1)-yearthai;
	document.getElementById('yearDropDown').style.display='none';
	//// fix for EI frame problem on time dropdowns 09/30/2006
				EIS_Hide_Frame();
...
function updateYearDiv()
{
	...
	for(var no=1;no<yearItems.length-1;no++){
		yearItems[no].innerHTML = currentYear/1 -yearSpan + no + yearthai;
...
function writeTopBar()
{
	...
	var span = document.createElement('SPAN');
	span.innerHTML = (currentYear*1)+yearthai;
...
function writeCalendarContent()
{
	...
	document.getElementById('calendar_year_txt').innerHTML = (currentYear*1)+yearthai;
...
function pickDate(e,inputDay)
{
	...
	currentYear = currentYear + yearthai;
...
function writeBottomBar()
{
	...
	bottomString = bottomString.replace('[year]',(d.getFullYear()*1)+yearthai);
...
function writeClearBar(element)
{
	...
	container.className = 'todaysDate';
	...
	clearSelected.onclick = function() {  returnDateTo.value=''; closeCalendar();};
...
function displayCalendar(inputField,format,buttonObj,displayTime,timeInput)
{
	...
	if(inputField.value.length>6){ //dates must have at least 6 digits...
       if(!inputField.value.match(/^[0-9]*?$/gi)){
   		...
   		currentYear = currentYear - yearthai;    // Modify select date
